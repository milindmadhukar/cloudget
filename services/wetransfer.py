#!/usr/bin/env python3
"""
WeTransfer download service implementation
"""
...
class WeTransferService(BaseDownloadService):
    """WeTransfer download service"""
    ...
    def convert_to_download_url(self, url: str) -> str:
        """Convert WeTransfer share URL to direct download URL"""
        if not self.is_supported_url(url):
            raise ValueError("Not a valid WeTransfer URL")
        
        # WeTransfer URLs need API interaction to get download links
        # For now, return the original URL and handle the conversion in get_file_info
        return url
    ...
    def extract_filename(self, url: str, response_headers=None) -> str:
        ...
        # For WeTransfer, we usually need to get filename from the API
        return 'wetransfer_file'
    
    async def get_file_info(self, session, url: str):
        """Get file info with special handling for WeTransfer API"""
        try:
            # WeTransfer requires special handling to get the actual download URL
            download_info = await self._get_wetransfer_download_info(session, url)
            
            if download_info:
                download_url = download_info['download_url']
                filename = download_info.get('filename', 'wetransfer_file')
                
                headers = self.get_default_headers()
                async with session.head(download_url, allow_redirects=True, headers=headers) as response:
                    response.raise_for_status()
                ...
                raise ValueError("Could not get WeTransfer download information")
                
        except Exception as e:
            self.logger.error(f"Failed to get file info: {e}")
            raise
    
    async def _get_wetransfer_download_info(self, session, url: str) -> Optional[dict]:
        """Get WeTransfer download information using their API"""
        try:
            # Extract transfer ID from URL
            transfer_id = self._extract_transfer_id(url)
            if not transfer_id:
                return None
            ...
            async with session.get(f"https://wetransfer.com/api/v4/transfers/{transfer_id}", headers=headers) as response:
                if response.status == 200:
                    transfer_data = await response.json()
                    
                    # Get the first file's download information
                    if 'files' in transfer_data and transfer_data['files']:
                        file_info = transfer_data['files'][0]
                        
                        # Request download URL
                        download_payload = {
                            'intent': 'entire_transfer',
                            'security_hash': transfer_data.get('security_hash', '')
                        }
                        
                        async with session.post(api_url, json=download_payload, headers=headers) as download_response:
                            if download_response.status == 200:
                                download_data = await download_response.json()
                                return {
                                    'download_url': download_data.get('direct_link'),
                                    'filename': file_info.get('name', 'wetransfer_file')
                                }
                                
        except Exception as e:
            self.logger.warning(f"Could not get WeTransfer download info via API: {e}")
        
        return None
    
    def _extract_transfer_id(self, url: str) -> Optional[str]:
        """Extract transfer ID from WeTransfer URL"""
        # Pattern for we.tl short URLs
        match = re.search(r'we\.tl/([a-zA-Z0-9]+)', url)
        if match:
            return match.group(1)
        
        # Pattern for full wetransfer.com URLs
        match = re.search(r'wetransfer\.com/downloads/([a-zA-Z0-9]+)', url)
        if match:
            return match.group(1)
    ...
    async def download_file(self, url: str, output_dir: str = ".", 
                          custom_filename: Optional[str] = None, custom_path: Optional[str] = None,
                          resume: bool = True, verify_hash: Optional[str] = None) -> str:
        """Override download_file to handle WeTransfer's special requirements"""
        try:
            async with aiohttp.ClientSession(
                connector=self.connector,
                timeout=self.client_timeout,
                ...
                # Get download information from WeTransfer API
                download_info = await self._get_wetransfer_download_info(session, url)
                if not download_info:
                    raise ValueError("Could not get WeTransfer download information")
                
                download_url = download_info['download_url']
                detected_filename = download_info['filename']
                
                self.logger.info(f"Converted URL: {download_url}")
                
                # Get file information
                file_size, filename, supports_ranges = await self.get_file_info(session, url)
                
                # Use detected filename if no custom filename provided
                if not custom_filename:
                    custom_filename = detected_filename
                ...
                # Setup progress bar
                display_name = output_path.name if custom_path else (custom_filename or filename)
                progress_bar = TqdmClass(
                    total=file_size,
                    unit='B',
                    unit_scale=True,
